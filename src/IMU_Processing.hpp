...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N) {
    ...
    if (b_first_frame_) {
        ...
        mean_acc << imu_acc.x, imu_acc.y, imu_acc.z + 9.81;
    ...
    for (const auto &imu: meas.imu) {
        ...
        cur_acc << imu_acc.x, imu_acc.y, imu_acc.z + 9.81;
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    ...
    v_imu_ = meas.imu;
    v_imu_.push_front(last_imu_);
    const double &imu_beg_time = v_imu_.front()->header.stamp.toSec();
    const double &imu_end_time = v_imu_.back()->header.stamp.toSec();
  ...
  //          <<meas.imu.size()<<" imu msgs from "<<imu_beg_time<<" to "<<imu_end_time<<endl;
    ...
    input_ikfom in;
    ROS_WARN_STREAM("Size Imu buffer: " << v_imu_.size() << "\n");
    for (auto it_imu = v_imu_.begin(); it_imu < (v_imu_.end() - 1); it_imu++) {
        auto &&head = *(it_imu);
        auto &&tail = *(it_imu + 1);
        ...
        acc_avr << 0.5 * (head->linear_acceleration.x + tail->linear_acceleration.x),
                ...
                0.5 * (head->linear_acceleration.z + 9.81 + tail->linear_acceleration.z + 9.81);
...
//      V3D P_compensate = imu_state.offset_R_L_I.conjugate() * (imu_state.rot.conjugate() * (R_i * (imu_state.offset_R_L_I * P_i + imu_state.offset_T_L_I) + T_i) - imu_state.pos) - imu_state.offset_T_L_I;
